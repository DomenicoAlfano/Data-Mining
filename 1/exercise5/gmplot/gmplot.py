...
class GoogleMapPlotter(object):
    ...
    def write_points(self, f, data):
        plus1 = 0
        minus1 = 0
        for point in self.points:
            self.write_point(f, point[0], point[1], point[2], point[3], plus1, data, minus1)
            plus1 += 1
            if plus1 > 1:
                minus1 = 2
    ...
    def write_point(self, f, lat, lon, color, title, plus1, data, minus1):
        f.write('\t\tvar latlng = new google.maps.LatLng(%f, %f);\n' %
                (lat, lon))
        f.write('\t\tvar img = new google.maps.MarkerImage(\'%s\');\n' %
                (self.coloricon % color))
        f.write('\t\tvar infowindow'+str(plus1)+' = new google.maps.InfoWindow({content: "<span>\'%s\'</span>"});\n' % data[plus1-minus1])
        f.write('\t\tvar marker'+str(plus1)+' = new google.maps.Marker({\n')
        f.write('\t\ttitle: "%s",\n' % title)
        f.write('\t\ticon: img,\n')
        f.write('\t\tposition: latlng\n')
        f.write('\t\t});\n')
        f.write('\t\tgoogle.maps.event.addListener(marker'+str(plus1)+', "mouseover", function() {infowindow'+str(plus1)+'.open(map,marker'+str(plus1)+');});\n')
        f.write('\t\tmarker'+str(plus1)+'.setMap(map);\n')
        f.write('')
